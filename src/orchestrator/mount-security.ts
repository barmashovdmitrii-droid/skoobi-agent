...
import { GROUPS_DIR, MOUNT_ALLOWLIST_PATH, STORE_DIR } from './config.js';
...
function isNonMainSensitiveProjectPath(realPath: string): boolean {
  const realGroups = getRealPath(GROUPS_DIR);
  const realStore = getRealPath(STORE_DIR);
  if (realGroups && isWithinPath(realGroups, realPath)) return true;
  if (realStore && isWithinPath(realStore, realPath)) return true;
  return false;
}
