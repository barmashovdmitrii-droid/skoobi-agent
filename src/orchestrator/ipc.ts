...
export function validateIpcSendFilePath(input: {
  ...
  const realPath = realpathOrNull(input.filePath);
  ...
  if (!input.isMain) {
    const groupRoot = realpathOrNull(
      path.resolve(GROUPS_DIR, input.sourceGroup),
    );
    if (!groupRoot || !isWithinPath(groupRoot, realPath)) {
    ...
    if (
      ...
      path.basename(realPath) === 'messages.db'
    ) {
            ...
            try {
              ...
              if (data.type === 'message' && data.chatJid && data.text) {
                ...
                if (
                  ...
                  const ok = await deps.router.sendPhoto(
                    data.chatJid,
                ...
                if (
                  ...
                  const ok = await deps.router.sendDocument(
                    data.chatJid,
                    fileValidation.realPath,
                    data.caption,
                  );
