...
describe('formatMessages', () => {
  ...
  it('can anonymize sender names for guest prompts', () => {
    const result = formatMessages(
      [
        makeMsg({ sender_name: 'Dmitry', content: 'hello' }),
        makeMsg({
          sender_name: 'Skoobi',
          content: 'hi',
          is_from_me: true,
    ...
    expect(result).not.toContain('Dmitry');
    expect(result).not.toContain('Skoobi');
  });
...
describe('prependRecentConversationContext', () => {
  const TZ = 'UTC';

  it('adds prior same-chat context before the new prompt', () => {
    const prompt = formatMessages([makeMsg({ content: 'what about it?' })], TZ);
    const result = prependRecentConversationContext(
      prompt,
      [
        makeMsg({ id: 'old-1', content: 'uploaded file' }),
        makeMsg({
          id: 'old-2',
          content: 'I summarized it',
          is_from_me: true,
        }),
      ],
      TZ,
    ...
    expect(result).toContain('<recent_conversation_context>');
    expect(result).toContain('uploaded file');
    expect(result).toContain('I summarized it');
    expect(result).toContain('sender="Assistant"');
    expect(result.indexOf('uploaded file')).toBeLessThan(
      result.indexOf('what about it?'),
    );
  });

  it('returns the prompt unchanged when there is no recent context', () => {
    const prompt = 'new prompt';
    expect(prependRecentConversationContext(prompt, [], TZ)).toBe(prompt);
  });
});
