...
import {
  createCodexSubscriptionModelGateway,
...
export type ModelRequest = {
  ...
  metadata: {
    ...
    image_paths?: string[];
    web_search_context_provided?: boolean;
    web_search_provider?: string;
    web_search_result_count?: number;
    voice_reply_requested?: boolean;
  };
};
...
export type ModelGatewayConfig = {
  type: 'openai_compatible' | 'codex_subscription_cli' | 'ollama';
  baseUrl: string;
  apiKey?: string;
...
export function loadModelGatewayConfig(
  ...
  const env = readEnvFile([
    ...
    'SKOOBI_CODEX_WEB_SEARCH_ENABLED',
  ...
  const codex =
    ...
    {};
  const envRoleOverrides: Partial<Record<ModelRole, string>> = {};
  const degradedModel = stringFrom(env.SKOOBI_QUOTA_DEGRADED_MODEL);
  if (degradedModel) envRoleOverrides.cheap = degradedModel;
  ...
  // Ollama preset: an OpenAI-compatible endpoint with local defaults — base
  // URL http://127.0.0.1:11434/v1 and (when SKOOBI_OLLAMA_MODEL is set) one
  // local model serving every role unless a role is mapped explicitly.
  const ollamaModel = stringFrom(env.SKOOBI_OLLAMA_MODEL);
  const ollamaRoleDefaults: Partial<Record<ModelRole, string>> =
    resolvedType === 'ollama' && ollamaModel
      ? (Object.fromEntries(
          (Object.keys(DEFAULT_ROLES) as ModelRole[]).map((role) => [
            role,
            ollamaModel,
          ]),
  ...
  return {
    ...
    baseUrl: normalizeBaseUrl(
      overrides.baseUrl ||
        ...
        (resolvedType === 'ollama'
          ? stringFrom(env.SKOOBI_OLLAMA_BASE_URL, OLLAMA_DEFAULT_BASE_URL)
          : ''),
    ),
    apiKey,
    roles: {
    ...
    codex: {
      ...
      model: stringFrom(
        ...
        stringFrom(codex.model, 'gpt-5.6-sol'),
      ),
      fallbackModel: stringFrom(
        env.SKOOBI_CODEX_FALLBACK_MODEL,
        stringFrom(
          codex.fallback_model ?? codex.fallbackModel,
          'gpt-5.6-terra',
        ),
      ),
      ...
      webSearchEnabled: boolFrom(
        env.SKOOBI_CODEX_WEB_SEARCH_ENABLED ??
          codex.web_search_enabled ??
...
export function createModelGateway(
  ...
  if (config.type === 'codex_subscription_cli') {
    return createCodexSubscriptionModelGateway({
      ...config.codex,
      roles: config.roles,
    });
  }
  const custom = resolveCustomProviderFactory(config.type);
  if (custom) return custom(config);
  // 'openai_compatible' and the 'ollama' preset share the same HTTP transport;
  // the preset only changes config defaults (base URL, role→model mapping).
