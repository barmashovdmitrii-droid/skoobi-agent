...
const storeMessageMock = vi.mocked(storeMessage);
...
describe('MessageIngestion', () => {
  ...
  it('generates collision-proof message ids for same-millisecond deliveries', async () => {
    // Freeze the clock so both deliveries see the identical Date.now() value.
    // Before the fix the id was `${triggerType}-${Date.now()}`, so the two
    // ids would be identical and storeMessage's INSERT OR REPLACE would
    ...
    try {
      ...
      expect(storeMessageMock).toHaveBeenCalledTimes(2);
      const firstId = storeMessageMock.mock.calls[0][0].id;
      const secondId = storeMessageMock.mock.calls[1][0].id;
      expect(firstId).not.toBe(secondId);
      // Readable trigger-type prefix is preserved.
      expect(firstId.startsWith('webhook-')).toBe(true);
      expect(secondId.startsWith('webhook-')).toBe(true);
    } finally {
      vi.useRealTimers();
    }
  });
