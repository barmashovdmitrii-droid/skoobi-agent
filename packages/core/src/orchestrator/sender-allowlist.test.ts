...
describe('loadSenderAllowlist', () => {
  ...
  it('fails CLOSED (deny-all/drop) on invalid JSON of an existing file', () => {
    const p = cfgPath();
    fs.writeFileSync(p, '{ not valid json }}}');
    const cfg = loadSenderAllowlist(p);
    // A present-but-corrupt config must NOT silently re-open the gate.
    expect(cfg.default.allow).toEqual([]);
    expect(cfg.default.mode).toBe('drop');
  });

  it('fails CLOSED (deny-all/drop) on invalid schema of an existing file', () => {
  ...
  it('rejects non-string allow array items by failing CLOSED', () => {
    const p = writeConfig({
      default: { allow: [123, null, true], mode: 'trigger' },
      chats: {},
    });
    const cfg = loadSenderAllowlist(p);
    // Invalid default → deny posture, not allow-all.
    expect(cfg.default.allow).toEqual([]);
    expect(cfg.default.mode).toBe('drop');
  });
...
// Fail-open vs fail-closed regression coverage.
// These use a freshly re-imported module so the internal last-known-good
// cache starts empty and behavior is deterministic regardless of test order.
describe('loadSenderAllowlist fail-closed posture', () => {
  let load: (p?: string) => SenderAllowlistConfig;

  beforeEach(async () => {
    vi.resetModules();
    const mod = await import('./sender-allowlist.js');
    load = mod.loadSenderAllowlist;
  });

  it('missing file fails OPEN (allow-all) — absent != corrupt', () => {
    const cfg = load(cfgPath('does-not-exist.json'));
    expect(cfg.default.allow).toBe('*');
    expect(cfg.default.mode).toBe('trigger');
  });

  it('present-but-corrupt config fails CLOSED to deny-all/drop (no prior good load)', () => {
    const p = cfgPath();
    fs.writeFileSync(p, 'totally not json');
    const cfg = load(p);
    expect(cfg.default.allow).toEqual([]);
    expect(cfg.default.mode).toBe('drop');
  });

  it('unreadable existing config (directory in place of file) fails CLOSED', () => {
    // A path that exists but cannot be read as a file yields a non-ENOENT
    // error (EISDIR); this must fail closed, not open.
    const p = cfgPath('as-dir');
    fs.mkdirSync(p);
    const cfg = load(p);
    expect(cfg.default.allow).toEqual([]);
    expect(cfg.default.mode).toBe('drop');
  ...
  it('fails CLOSED to deny-all on later corruption (does not trust a stale in-memory policy)', () => {
    const p = cfgPath();
    // First a valid policy loads...
    fs.writeFileSync(
      p,
      JSON.stringify({ default: { allow: ['alice'], mode: 'drop' }, chats: {} }),
    );
    expect(load(p).default.allow).toEqual(['alice']);

    // ...then the file is corrupted in place. We deliberately do NOT retain the
    // prior in-memory config (it could just as easily have been allow:'*', which
    // would silently fail OPEN): a present-but-corrupt config fails CLOSED to
    // deny-all/drop.
    fs.writeFileSync(p, '{ corrupt ]');
    const afterCorruption = load(p);
    expect(afterCorruption.default.allow).toEqual([]);
    expect(afterCorruption.default.mode).toBe('drop');
  });
});
