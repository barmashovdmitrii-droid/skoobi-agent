...
function estimateCost(
  ...
  // Cache-creation (write) tokens have no dedicated rate here, so they are
  // priced at the base input rate — matching quota.ts calculateCredits.
  return (
    (inputTokens / 1_000_000) * tier.input +
    (outputTokens / 1_000_000) * tier.output +
    (cacheReadTokens / 1_000_000) * tier.cacheRead +
    (cacheCreationTokens / 1_000_000) * tier.input
  );
}
